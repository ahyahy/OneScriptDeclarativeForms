...
namespace osdf
{
    [ContextClass("ДфГруппаКолонок", "DfColgroup")]
    public class DfColgroup : AutoContext<DfColgroup>
    {

        public DfColgroup()
        {
            ItemKey = "d" + Path.GetRandomFileName().Replace(".", "");
            string strFunc = "mapKeyEl.set('" + ItemKey + "', document.createElement('colgroup'));" + @"
mapElKey.set(mapKeyEl.get('" + ItemKey + "'), '" + ItemKey + "');";
        ...
        public int Span
        {
            ...
            {
                span = value;
                string strFunc = "mapKeyEl.get('" + ItemKey + "')['span'] = '" + span + "';";
                DeclarativeForms.SendStrFunc(strFunc);
            }
        }
