...
namespace osdf
{
    [ContextClass("ДфУпорядоченныйСписок", "DfOrderedList")]
    public class DfOrderedList : AutoContext<DfOrderedList>
    {

        public DfOrderedList()
        {
            ItemKey = "d" + Path.GetRandomFileName().Replace(".", "");
            string strFunc = "mapKeyEl.set('" + ItemKey + "', document.createElement('ol'));" + @"
mapElKey.set(mapKeyEl.get('" + ItemKey + "'), '" + ItemKey + "');";
        ...
        public int Start
        {
            ...
            {
                start = value;
                string strFunc = "mapKeyEl.get('" + ItemKey + "')['start'] = '" + start + "';";
                DeclarativeForms.strFunctions = DeclarativeForms.strFunctions + strFunc + DeclarativeForms.funDelimiter;
            }
        }

        private bool reversed;
        [ContextProperty("Обратный", "Reversed")]
        public bool Reversed
        {
            ...
            {
                reversed = value;
                string strFunc = "mapKeyEl.get('" + ItemKey + "')['reversed'] = " + reversed.ToString().ToLower() + ";";
                DeclarativeForms.strFunctions = DeclarativeForms.strFunctions + strFunc + DeclarativeForms.funDelimiter;
            }
        }
        ...
        private string type;
        [ContextProperty("ТипМаркера", "Type")]
        public string Type
        {
            get { return type; }
            set
            {
                type = value;
                string strFunc = "mapKeyEl.get('" + ItemKey + "')['type'] = '" + type + "';";
                DeclarativeForms.strFunctions = DeclarativeForms.strFunctions + strFunc + DeclarativeForms.funDelimiter;
            }
        }
